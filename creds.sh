...
set -eu -o pipefail
name=$(echo "$output" | tr // -)
OUTFILE="creds-${name}.txt"
declare -A DATA

rm -rf "$OUTFILE"

SSM_PARAMS=("DATABASE_HOST" "DATABASE_PORT" "DATABASE_PASSWORD" "DATABASE_USERNAME" "REDIS_HOST" "REDIS_PORT")
for x in "${SSM_PARAMS[@]}"
do
...
PGPASSWORD="${DATA['DATABASE_PASSWORD']}" createdb -h "${DATA['DATABASE_HOST']}" -p "${DATA['DATABASE_PORT']}" -U "${DATA['DATABASE_USERNAME']}" "${OUT['DATABASE_NAME']}"
PGPASSWORD="${DATA['DATABASE_PASSWORD']}" createuser -h "${DATA['DATABASE_HOST']}" -p "${DATA['DATABASE_PORT']}" -U "${DATA['DATABASE_USERNAME']}" "${OUT['DATABASE_USER']}"
PGPASSWORD="${DATA['DATABASE_PASSWORD']}" psql -h "${DATA['DATABASE_HOST']}" -p "${DATA['DATABASE_PORT']}" -U "${DATA['DATABASE_USERNAME']}" -c "ALTER USER ${OUT['DATABASE_USER']} WITH ENCRYPTED PASSWORD \'${OUT['DATABASE_PASS']}\'; GRANT ALL PRIVILEGES ON DATABASE ${OUT['DATABASE_NAME']} TO ${OUT['DATABASE_USER']};"
